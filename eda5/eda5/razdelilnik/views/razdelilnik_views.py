...
# Models
from ..models import Razdelilnik, RacunRazdelilnik
...
class RazdelilnikDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['racunrazdelilnik_list'] = RacunRazdelilnik.objects.filter(
            razdelilnik=self.object.id).annotate(vrednost_osnova=Sum('racun__strosek__osnova'))

        vrednost_razdelilnik_osnova = RacunRazdelilnik.objects.filter(
            razdelilnik=self.object.id).aggregate(vrednost=Sum('racun__strosek__osnova'))

        context['vrednost_razdelilnik_osnova'] = vrednost_razdelilnik_osnova

        vrednost_razdelilnik_ddvbrez = RacunRazdelilnik.objects.filter(
            razdelilnik=self.object.id, racun__strosek__stopnja_ddv=0).aggregate(vrednost=Sum('racun__strosek__osnova')*1)

        vrednost_razdelilnik_ddvnizja = RacunRazdelilnik.objects.filter(
            razdelilnik=self.object.id, racun__strosek__stopnja_ddv=1).aggregate(vrednost=Sum('racun__strosek__osnova')*1.095)

        vrednost_razdelilnik_ddvvisja = RacunRazdelilnik.objects.filter(
            razdelilnik=self.object.id, racun__strosek__stopnja_ddv=2).aggregate(vrednost=Sum('racun__strosek__osnova')*1.22)
